...
class BilevelConfig(OptimizerConfig):
    ...
    def reporter(self) -> ActorHandle[WandbReporter] | None:
        """Handle of the ``WandbReporter`` actor, ``None`` until :meth:`build_optimizer` creates it.

        The actor is only created when :meth:`reporting` selected Weights &
        Biases; without a reporting call the handle stays ``None``.
        """
