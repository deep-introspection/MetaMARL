...
def plot_ecosystem_dynamics(
    ...
    for i, (ep, data) in enumerate(episodes.items()):
        ax1.plot(data["steps"], data["fish"], 
                color=colors[i], alpha=0.7, linewidth=1.5,
                label=f"Episode {ep}")
    
    if show_sustainability_threshold:
        ax1.axhline(y=sustainability_threshold, color='red', linestyle='--', 
                   linewidth=2, alpha=0.8, label=f'Collapse threshold ({sustainability_threshold})')
    ...
    for i, (ep, data) in enumerate(episodes.items()):
        ax2.plot(data["steps"], data["algae"], 
                color=colors[i], alpha=0.7, linewidth=1.5,
                label=f"Episode {ep}")
...
def plot_fishermen_actions(
    ...
    for i, (ep, data) in enumerate(episodes.items()):
        axes[0].plot(data["steps"], data["harvest"], 
                    color=colors[i], alpha=0.7, linewidth=2,
                    label=f"Episode {ep} - Harvest")
        axes[0].plot(data["steps"], data["quota"], 
                    color=colors[i], alpha=0.5, linestyle='--', linewidth=1,
    ...
    for i, (ep, data) in enumerate(episodes.items()):
        violation_steps = [s for s, v in zip(data["steps"], data["over_quota"]) if v > 0]
        violation_amounts = [v for v in data["over_quota"] if v > 0]
        
        if violation_steps:
            axes[1].scatter(violation_steps, violation_amounts, 
    ...
    for i, (ep, data) in enumerate(episodes.items()):
        ...
        axes[2].plot(data["steps"], harvest_rate, 
                    color=colors[i], alpha=0.7, linewidth=1.5,
                    label=f"Episode {ep}")
...
def plot_combined_trial_analysis(
    ...
    for i, (ep, data) in enumerate(episodes.items()):
        axes[0, 0].plot(data["steps"], data["fish"], 
                       color=colors[i], alpha=0.7, linewidth=1.5,
                       label=f"Episode {ep}")
    
    axes[0, 0].axhline(y=sustainability_threshold, color='red', linestyle='--', 
                      linewidth=2, alpha=0.8, label=f'Collapse threshold')
    ...
    for i, (ep, data) in enumerate(episodes.items()):
        axes[0, 1].plot(data["steps"], data["algae"], 
                       color=colors[i], alpha=0.7, linewidth=1.5,
                       label=f"Episode {ep}")
    ...
    for i, (ep, data) in enumerate(episodes.items()):
        axes[1, 0].plot(data["steps"], data["harvest"], 
                       color=colors[i], alpha=0.8, linewidth=2,
                       label=f"Episode {ep} - Harvest")
        axes[1, 0].plot(data["steps"], data["quota"], 
    ...
    for i, (ep, data) in enumerate(episodes.items()):
        scatter = axes[1, 1].scatter(data["algae"], data["fish"], 
                                   c=data["harvest"], cmap='viridis', 
                                   alpha=0.6, s=30,
                                   label=f"Episode {ep}")
...
def load_and_visualize_experiment(
    ...
    trace_files = [
        f for f in os.listdir(f"{experiment_dir}/outer_{outer_iter}")
        if f.startswith("best_trace_") and f.endswith(".json")
    ]
    
    if not trace_files:
        raise FileNotFoundError(f"No trajectory files found in {experiment_dir}/outer_{outer_iter}")
    ...
    try:
        ...
        if headers and rows:
            ...
            mechanism_params = {
                k: float(v) for k, v in best_candidate.items()
                if k in ['fixed_quota', 'prop_quota', 'min_stock', 'fine_amount', 'ban_period']
            }
        else:
    ...
    figures["ecosystem"] = plot_ecosystem_dynamics(
        ...
        save_path=f"{experiment_dir}/outer_{outer_iter}/ecosystem_dynamics.png" if save_plots else None
    )
    ...
    figures["actions"] = plot_fishermen_actions(
        ...
        save_path=f"{experiment_dir}/outer_{outer_iter}/fishermen_actions.png" if save_plots else None
    )
    ...
    figures["combined"] = plot_combined_trial_analysis(
        ...
        save_path=f"{experiment_dir}/outer_{outer_iter}/combined_analysis.png" if save_plots else None
    )
...
def create_experiment_summary_visualization(
    ...
    objective_scores = [float(row.get("best_iteration_score", 0)) for row in summary_data]
    ...
    sustainability_penalties = [float(row.get("sustainability_penalty", 0)) for row in summary_data]
    ...
    axes[0, 1].plot(iterations, mean_rewards, 'o-', color='green', linewidth=2, markersize=6)
    ...
    axes[1, 0].plot(iterations, collapse_rates, 'o-', color='red', linewidth=2, markersize=6)
    ...
    axes[1, 1].plot(iterations, sustainability_penalties, 'o-', color='orange', linewidth=2, markersize=6)
...
def plot_mechanism_parameter_analysis(
    ...
    mechanism_params = ['fixed_quota', 'prop_quota', 'min_stock', 'fine_amount', 'ban_period']
    
    # Convert to float arrays
    param_data = {param: [float(c[param]) for c in all_candidates] for param in mechanism_params}
    objective_scores = [float(c['objective_score']) for c in all_candidates]
    mean_rewards = [float(c['mean_reward']) for c in all_candidates]
    sustainability_penalties = [float(c['sustainability_penalty']) for c in all_candidates]
    ...
    for i, param in enumerate(mechanism_params):
        ...
        if row < 2:  # First 4 parameters in top 2 rows
            sc = axes[row, col].scatter(param_data[param], objective_scores, **scatter_kwargs)
            axes[row, col].set_xlabel(param.replace('_', ' ').title())
            axes[row, col].set_ylabel('Objective Score')
            axes[row, col].set_title(f'Objective Score vs {param.replace("_", " ").title()}')
            axes[row, col].grid(True, alpha=0.3)
    
    # Handle 5th parameter (ban_period) in bottom left
    sc = axes[2, 0].scatter(param_data['ban_period'], objective_scores, **scatter_kwargs)
...
def plot_pareto_frontier_analysis(
    ...
    scatter = ax1.scatter(sustainability_penalties, mean_rewards, 
                         c=objective_scores, cmap='viridis', alpha=0.6, s=50)
    ...
    ax2.scatter(iterations, objective_scores, 
               c=objective_scores, cmap='viridis', alpha=0.6, s=50)
...
def plot_mechanism_parameter_pairs(
    ...
    # Extract mechanism parameters and metrics
    mechanism_params = ['fixed_quota', 'prop_quota', 'min_stock', 'fine_amount', 'ban_period']
    param_labels = ['Fixed Quota', 'Prop Quota', 'Min Stock', 'Fine Amount', 'Ban Period']
        ...
        for j, param_y in enumerate(mechanism_params):
            ...
            if i == j:
                ...
                for k in range(len(bins)-1):
                    mask = (np.array(param_data[param_x]) >= bins[k]) & (np.array(param_data[param_x]) < bins[k+1])
                    if np.any(mask):
                ...
                bars = ax.bar(bin_centers, counts, width=bins[1]-bins[0], 
                             alpha=0.7, edgecolor='black', linewidth=0.5)
                
                # Color bars based on bin scores
                if bin_scores:
                    norm = plt.Normalize(vmin=min(objective_scores), vmax=max(objective_scores))
                ...
                sc = ax.scatter(param_data[param_x], param_data[param_y], **scatter_kwargs)
    ...
    plt.suptitle('Mechanism Parameter Pairwise Analysis - All Tested Candidates', fontsize=16)
    plt.tight_layout()
