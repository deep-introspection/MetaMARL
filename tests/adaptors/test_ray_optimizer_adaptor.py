...
# --------------------------------------------------------------------------- #
# Helpers kept for a future design (unused in the current loop)
# --------------------------------------------------------------------------- #


@pytest.mark.unit
def test_build_agent_policy_map_expands_counts():
    specs = {
        "fisher": {"count": 2, "policy": "fisher"},
        "patrol": {"count": 1, "policy": "shared"},
    }
    cfg = make_config(agent_specs=specs)
    opt = RayOptimizer.__new__(RayOptimizer)
    opt.config = cfg

    assert opt._build_agent_policy_map() == {
        "fisher:0": "fisher",
        "fisher:1": "fisher",
        "patrol:0": "shared",
    }
...
@pytest.mark.unit
def test_get_policy_handle_prefers_rl_module_then_policy():
    opt = RayOptimizer.__new__(RayOptimizer)
    opt.algo = SimpleNamespace(
        get_module=lambda pid: f"module:{pid}", get_policy=lambda pid: f"policy:{pid}"
    )
    assert opt._get_policy_handle("p") == "module:p"

    def no_module(pid):
        raise AttributeError("old stack")

    opt.algo = SimpleNamespace(
        get_module=no_module, get_policy=lambda pid: f"pol:{pid}"
    )
    assert opt._get_policy_handle("p") == "pol:p"


@pytest.mark.unit
def test_get_policy_handle_without_algo_raises_attribute_error(stub_actor_class):
    # Documented dead code: ``RayOptimizer`` never defines ``self.algo``.
    opt = RayOptimizer(config=make_config())
    with pytest.raises(AttributeError):
        opt._get_policy_handle("p")
