...
@pytest.mark.unit
def test_payload_fixed_mode_uses_template_vector_with_positional_names():
    """A fixed template optimizes nothing but still exposes a full vector.

    ``param_names`` only covers the encoded (optimized) entries, so the fixed
    vector is logged under positional names.
    """
    ...
    opt.env = SimpleNamespace(mechanism_template=mechanism)
    payload = opt._to_logger_payload(
        inner=InnerSchema(),
