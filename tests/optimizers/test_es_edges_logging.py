...
class _FakeRegulatorEnv:
    def __init__(self, dimension, fitness_fn):
        # The optimizer reads the optimized parameter names from the env's
        # mechanism template; a fixed template (dimension 0) has none but
        # still exposes a full vector, logged under positional names.
        names = [f"p{i}" for i in range(dimension)]
        self.mechanism_template = SimpleNamespace(
            param_names=lambda: names,
            to_vector=lambda: (
...
class TestRun:
    ...
    def test_fixed_mode_run_plots_template_vector(self, fake_reporter):
        ...
        opt.run()
        peeked = calls[0]
        assert set(peeked.search_mean) == {"parameter_0", "parameter_1"}
