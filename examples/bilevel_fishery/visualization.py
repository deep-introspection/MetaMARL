...
def plot_optimization_tradeoff(
        sustainability_penalities: list[float],
        mean_rewards: list[float],
        save_path: Optional[str] = None,
) -> plt.Figure:
    # TODO save_path2 = output_path / f"opt_{iteration:03d}.png"
    if save_path is None:
        save_path = Path(__file__).resolve().parents[2] / f"/results/optimization_tradedoff/"
    fig, ax = plt.subplots()
    ax.plot(sustainability_penalities, mean_rewards)

    ax.set_xlabel("Sustainability Penalties")
    ax.set_ylabel("Mean Rewards")
    ax.set_title("Sustainability vs Market Value")
