...
def evaluate_mechanism_with_metrics(
    ...
    for episode_idx in range(num_episodes):
        ...
        while not (any(terminated.values()) or any(truncated.values())):
            ...
            # Execute step
            observations, rewards, terminated, truncated, infos = environment.step(agent_actions)
    ...
    sustainability_penalties = [
        max(0.0, (sustainability_threshold - min_fish) / max(EPS, sustainability_threshold))
...
def evaluate_multiple_episodes(
    ...
    metrics, trajectories = evaluate_mechanism_with_metrics(
        algorithm, environment, num_episodes, sustainability_threshold, 
...
def compute_sustainability_score(
    ...
    collapsed_steps = sum(1 for pop in fish_populations if pop < sustainability_threshold)
