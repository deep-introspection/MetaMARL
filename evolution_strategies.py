...
from config import ES_CONFIG, EPS
...
class EvolutionStrategies:
    ...
    def update_parameters(self, population: np.ndarray, fitness_scores: List[float]) -> None:
        ...
        fitness_ranks = np.argsort(np.argsort(-fitness_array))  # Higher rank = better fitness
        ...
        sigma_multiplier = math.exp(
            self.sigma_lr * (fitness_std - target_diversity)
        )
...
def update_es_parameters(
    ...
    target_diversity = 1e-3  # Target minimum diversity
    sigma_multiplier = math.exp(
        sigma_learning_rate * (fitness_std - target_diversity)
    )
    
    # Update sigma with bounds
    new_sigma = np.clip(
        current_sigma * sigma_multiplier,
        ES_CONFIG["min_sigma"],
        ES_CONFIG["max_sigma"]
    )
