...
class FisheryEnvFixed(MultiAgentEnv):
    ...
    def reset(
        ...
        *, 
        ...
        self._algae_population = max(EPS, float(rng.lognormal(mean=algae_mean, sigma=0.05)))
        self._fish_population = max(EPS, float(rng.lognormal(mean=fish_mean, sigma=0.05)))
        ...
        observations = {
            agent_id: np.array(
                [self._algae_population, self._fish_population], 
                dtype=np.float32
            ) 
            for agent_id in self.fishermen
        }
    ...
    def step(
        ...
        is_truncated = {agent_id: self._time_step >= self.horizon for agent_id in self.fishermen}
