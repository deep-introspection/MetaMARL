...
def list_available_experiments() -> None:
    ...
    for i, exp_dir in enumerate(all_dirs):
        ...
        mtime_str = (
            os.popen(f'stat -f "%Sm" -t "%Y-%m-%d %H:%M:%S" "{exp_dir}"').read().strip()
        )
...
def main():
    ...
    group.add_argument(
        "--iteration", type=int, help="Specific outer iteration to visualize"
    )
    group.add_argument(
        "--all-iterations",
        action="store_true",
    ...
    try:
        ...
        if not args.iteration and not args.all_iterations:
            ...
            try:
                ...
                print(
                    f"✓ Summary visualization saved: {os.path.join(output_dir, 'experiment_summary.png')}"
                )
        ...
        if os.path.exists(args.experiment_dir):
            for item in os.listdir(args.experiment_dir):
                if item.startswith("outer_") and os.path.isdir(
                    os.path.join(args.experiment_dir, item)
                ):
                    try:
                        ...
                        has_traces = any(
                            f.startswith("best_trace_") and f.endswith(".json")
        ...
        if not available_iterations:
            ...
            print(
                "Run experiments with --trace-episodes > 0 to generate trajectory data for visualization."
            )
            if success_count == 0:
                sys.exit(1)
            return
        ...
        if args.iteration is not None:
            if args.iteration in available_iterations:
                ...
                print(
                    f"Error: Iteration {args.iteration} not found or has no trajectory data"
                )
    ...
    if success_count > 0:
        ...
        if os.path.exists(os.path.join(output_dir, "experiment_summary.png")):
            print(
                f"  - Experiment summary: {os.path.join(output_dir, 'experiment_summary.png')}"
            )
